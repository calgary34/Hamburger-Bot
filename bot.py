...
async def eightball( ctx,*,question):
  ...
  em = discord.Embed(title="8ball",description=f'Your question: {question}\n8ball says: {random.choice(answers)}\n:8ball::8ball::8ball::8ball::8ball:', color=0x275ef4)
  await ctx.send(embed=em)
...
async def help(ctx):
  ...
  em.add_field(name="Random Commands", value="`random-integer`, `random-float`, `random-letter`, `random-shuffle`, `random-choice`, `8ball`,`dice`")
  await ctx.send(embed=em)
...
async def help_dice(ctx):
  cmd=HelpCommand("Help on `dice`","Rolls dice! You can specify the number of dice too.","`;dice <number of dice rolled (optional)>`","dices")
  await ctx.send(embed=cmd.em)
